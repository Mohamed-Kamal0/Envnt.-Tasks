...
export class ProductService {
  ...
  /**
   * DAY 5, TASK C — the edit form packages its values into this payload and
   * fires an HTTP PUT to update the product on the .NET API.
   *
   * #Task 3: Implement this method so it sends an HTTP PUT request to this
   * product's endpoint (the base URL followed by the product's id) with the
   * given payload as the request body, and returns the updated Product.
   * `payload` is a `ProductPayload` (the `Product` shape minus `id`, since the
   * id is already in the URL). The `of(...)` stub below is just a placeholder
   * so the app keeps compiling and running until you replace it.
   */
  update(id: number, payload: ProductPayload): Observable<ProductPayload> {
    return this.http
      .put<
  ...
  private unwrapOne(response: Product | Record<string, unknown>): Product {
    ...
    console.warn(
      "ProductService: unexpected product shape from API:",
      response,
    );
